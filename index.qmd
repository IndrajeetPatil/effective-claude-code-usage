...
<span style="color:#22c55e;">✓</span> Claude Code v2.x
...
| `/skillname` | Run a named skill (reusable prompt playbook) |
...
**Automatic memory** · Claude also builds persistent memories across sessions automatically — no `/memory` command needed. Opt out per-project in settings.
:::
...
CLAUDE.md for memory · `/plan` for complex tasks · Subagents for parallel work · `/compact` when context fills · CLI > MCP for simplicity · Intent + acceptance criteria over instructions · Hooks for automated validation
:::
